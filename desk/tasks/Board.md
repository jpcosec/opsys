...
- desk/tasks/task-reject-empty-and-whitespace-only-selectors-with-a-clear-message.md
...
- Reject empty and whitespace-only selectors with a clear message [draft] - Every selector-taking command rejects an empty or whitespace-only selector before any lookup, naming the command and the subject.
- Add deskops desk update to reconcile a desk and its store [draft] - One command reports and repairs the divergence between a desk, its tracked documents and the current models: missing structure, untracked documents, stale hashes, unregistered models, with a dry-run default and an explicit apply.
- Document the atom model and the tag namespace workflow [draft] - A durable doc, materializing the atoms that already carry these rules, that explains creating an atom, choosing the 5WH1+ question, how namespaces select the atom's folder, and how to add a namespace when existing ones do not cover the knowledge.
- Report knowledge drift: stale atoms, missing materializations and unrouted findings [draft] - A drift command that reports, from the desk and the graph: atoms whose declared targets or materializations no longer resolve, documents that point at atoms with no inbound trace, and graph findings that no atom or issue covers.
- Make cross-repo inbox delivery work from a canonical identity [draft] - One identity path usable for both questions, and cross-repo delivery that either reaches the target or fails with the reason, with the sender able to see that the target acknowledged or closed the note.
